...
export async function createTodo(todo) {
    const resp = await client.from('todos').insert({ todo, complete: false, user_id: client.auth.user().id, })
        .single();
...
export async function getTodos() {
    const resp = await client.from('todos').select().order('complete').match({ user_id: client.auth.user().id, });
    return checkError(resp);
}

export async function completeTodo(id) {
    const resp = await client.from('todos').update({ complete: true }).match({ user_id: client.auth.user().id, id: id });
    // console.log(resp);
